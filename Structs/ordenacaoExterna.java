...
public class ordenacaoExterna {
    ...
    private static filme getFilme() throws IOException{
        RandomAccessFile arq = new RandomAccessFile("../TP1/Banco/DB.db", "rw");
        filme f = null;
        int tamanhoRegistro;
        ...
        while(resp == false){
            if( pos < arq.length()){
                ...
                if(lapide == false){
                    ...
                    arq.skipBytes(tamanhoRegistro - 1);//pular o tamanho do registro que é uma lapide - 1 byte que ja foi lido
                    pos = (int) arq.getFilePointer();
                }
            }else{
                arq.close();
    ...
    public static void balanceadaComum(int M, int N) throws IOException {
        // M -> número máximo de registros que podem ser armazenados na memória principal
        // N -> número de caminhos
        String inputFile = "Banco/DB.db"; // arquivo de entrada
        ...
        while (true) {
            ...
            // Dividir o buffer em N caminhos e gravar os dados classificados em arquivos temporários
